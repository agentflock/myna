...
### Example 1 — technical plan / migration proposal (Key Assumptions Check + ACH + outside view)
**Setting:** A written plan titled "Migration to Vendor X"
**Finding:** The artifact's load-bearing assumptions are thinking-shaped, not implementation-shaped — and three of them are unexamined: (a) **KAC** — the assumption that the rejected alternative (status quo + targeted fixes) wouldn't have produced different success criteria; the plan's metrics were defined after the migration was already favored, which means the alternatives are being scored against a rubric built for the recommendation. What evidence would falsify this? An ex-ante list of success criteria written before the option set was narrowed. (b) **ACH** — the assumption that customer requests for "better integration" point at this vendor as the solution; the artifact treats those requests as confirming evidence, but the same requests are equally consistent with the status quo plus a thin adapter. Both hypotheses fit the data; the plan picks one without ruling out the other. (c) **Outside view** — the assumption that this team's reference class for vendor migrations is the recent successful ones; the artifact's confidence reads as inside-view. The base rate for vendor-migration projects of this shape across the org's last several attempts is the relevant prior, and it is not named. Each of these is a thinking move the artifact skipped, not an implementation detail it got wrong — name each with a confidence level and a falsification criterion for each.
