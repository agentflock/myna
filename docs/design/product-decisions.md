...
### D069 — Vault-native reminder model: typed checkbox, vault-first, fire-once
**Date:** 2026-06-13
**Context:** Several early skill designs bundled reminders into `/myna:block-time` — treating them as calendar events with an optional time. This conflated two different things: time blocks (calendar-centric, about protecting focus time) and reminders (vault-centric, about surfacing something on a future day). The daily brief already had a natural surfacing slot, and the vault's `[type:: reminder]` field made it possible to store and query reminders without a calendar dependency. The question was whether to keep reminders inside block-time or give them a distinct model with a dedicated skill.
**Decision:** Reminders are a first-class vault entity with their own type, file, and skill. Key properties: (1) **Typed checkbox** — stored as `- [ ] {what} 📅 {YYYY-MM-DD} [type:: reminder]`, distinct from `[type:: task]` so task queries never surface reminders accidentally. Type whitelists in steering-vault-ops include `reminder` only in the explicit reminder context, not in task queries. (2) **Vault-first** — reminder is always written to `{vault_path}/_system/data/reminders.md` before any calendar action. The write succeeds even when the calendar MCP is unavailable. (3) **Optional calendar push** — if the user gives a clock time, the skill confirms before creating a personal calendar event with title `[Myna:Reminder] {what}`, no attendees, 15-minute duration. If no time is given and the calendar MCP is available, the skill asks whether the user wants a notification. (4) **Fire-once via daily brief** — the daily brief surfaces all reminders where `due <= today` and marks them done (`[x]`) when the user acknowledges. No recurrence, no scheduling daemon. (5) **`_system/data/` folder** — reminders.md lives alongside the moved links.md (previously `_system/data/links.md`) in a dedicated data subfolder under `_system/`, keeping machine-written non-config state separate from config, logs, sources, and parked context. The skill is `/myna:reminder`; block-time no longer handles reminders.
**Alternatives rejected:** Keep reminders inside `/myna:block-time` (conflates calendar-centric and vault-centric concepts; block-time is about protecting time, not surfacing future notes; creates ambiguity when the calendar MCP is unavailable). Use `[type:: task]` with a reminder tag (task queries would surface all reminders — noise in every task view; the type whitelist approach is cleaner). Store reminders in a per-day daily note (can't be queried by due date across days; gets archived with the note; harder to manage from the reminder skill without reading many files). Implement recurrence (unnecessary complexity for v1; most reminders are one-time; recurrence can be added later when there's real demand).
