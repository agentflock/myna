...
- **meetings.yaml** — machine-managed state written by Myna's skills. Optional overrides for meeting type inference (D022). Most meetings don't need entries here; users do not edit this file directly
- **communication-style.yaml** — sign-off only. Drafting skills apply built-in tier-aware shaping (BLUF for upward, conversational for peer, etc.) without reading preset names. Dead fields (`default_preset`, `presets_per_tier`, `difficult_message_approach`, `email_preferences.*`, `messaging_preferences.*`) removed
- **tags.yaml** — custom keyword-based tags only. Project, person, and source tags are auto-derived from projects.yaml and people.yaml; this file is an extension point for tags not covered by those sources. Starts empty (`tags: []`)
...
- Tabs: Identity, Email, Communication, Projects, People, Files
