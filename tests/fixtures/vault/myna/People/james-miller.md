---
created: 2025-01-10
---

#person #upward

## Overview

**Name:** James Miller
**Role:** VP Engineering
**Team:** Engineering Leadership
**Relationship:** upward

## Communication Preferences

> Populated over time. What works, what doesn't.

- Wants BLUF — bottom line first, then evidence
- Reads written status updates more carefully than slides
- Prefers a one-pager monthly over weekly emails
- Direct question in a meeting means he wants a direct answer — no hedging
- Has a standing skip-level program — every IC on the team gets a 1:1 with him quarterly

## Observations

> Chronological log of observations. Each entry has type, area, and source.

- [2025-06-02 | meeting portfolio review] **strength:** Cut a pre-read from 12 pages to 2 and still landed every decision we needed [User]
- [2025-12-18 | email from James] **contribution:** Called out Sarah Carter by name for Compass launch — specific, well-written, got filed in her HR record [Auto]
- [2026-03-15 | email from James] **strength:** Wrote a detailed praise note for Sarah's incident handling, spelling out exactly what she did well [User]

## Pending Feedback

> Undelivered observations with coaching-tone talking points. Cleared when delivered.

## Recognition

> Accomplishments and recognition entries.

## Personal Notes

> Non-work details: hobbies, family, milestones.

- [2025-05-10] Three kids, youngest just started kindergarten — no meetings before 9 AM on drop-off days
- [2026-02-01] Has a standing skip-level program — every direct's report gets a 1:1 with him quarterly
- [2026-03-22] Into road cycling — mentioned an upcoming century ride in Napa

## Meeting History

> Links to meeting files with this person.

- [[Recurring/eng-leads-sync]] — Engineering leads sync
